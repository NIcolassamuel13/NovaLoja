...
namespace SalesWebMvc.Data
{
    ...
    // quando seedingService for criado automaticamente ele vai receber uma instância do SalesWebMvcContex também

    public class SeendingService
    {
        // criar injeção de dependencia da classe dbcontext
        private SalesWebMvcContext _context;
        public SeendingService(SalesWebMvcContext context)
        {
            _context = context;
        }
        // operação responsavel por popular minha base de dados
        public void Seed()
        {
            //aqui vou testar se há algum registro no banco par cada uma das classes
            if (_context.Department.Any() ||
                _context.Sellers.Any()
                || _context.SalesRecords.Any())
            {
                // caso existir vou dar um return que dizer que a base de dados já foi populado
                return;
            }
            ...
            // foi feita a inclusão dos objetos no banco de dados através do AddRanger
            ...
            //para efetivar essas alterações tenho que usasr o SaveChanges()
            //aqui salva e confirma a operação no banco de dados
            _context.SaveChanges();

            //após isso quenho que chamar o seedingService lá no startup dentro do metodo configure
